...
CamR= cv2.VideoCapture(1)   # 2 -> Right Camera
...
while True:
    ...
    if (retR == True) & (retL == True):
        ...
        if cv2.waitKey(0) & 0xFF == ord('s'):   # Push "s" to save the images and "c" if you don't want to
            ...
            cv2.imwrite('ImagesCalib/chessboard-R'+str_id_image+'.png',frameR) # Save the image in the file where this Programm is located
            cv2.imwrite('ImagesCalib/chessboard-L'+str_id_image+'.png',frameL)
            id_image=id_image+1
        else:
